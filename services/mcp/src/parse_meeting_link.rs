//! Faithful port of services/mcp/main.py's `_parse_meeting_url` — pure logic, no HTTP calls,
//! so it stays a direct port rather than a REST proxy like every other tool in this service.
...
pub fn parse_meeting_url(meeting_url: &str) -> Result<ParseMeetingLinkResponse, String> {
    ...
    if host == "meet.google.com" {
        ...
        let code = path.trim_matches('/').split('/').next().unwrap_or("").to_string();
    ...
    if host.ends_with("teams.live.com") {
        ...
        let Some(caps) = re.captures(&path) else {
            return Err("Unsupported teams.live.com URL format. Expected /meet/<10-15 digit id>.".to_string());
        };
    ...
    if is_teams_enterprise_host(&host) {
        ...
        if (path.trim_end_matches('/') == "/v2" || path.trim_end_matches('/').is_empty()) && fragment.starts_with("/meet/") {
            if let Ok(frag_url) = Url::parse(&format!("https://x{fragment}")) {
                if let Some(caps) = re.captures(frag_url.path()) {
                    let native_id = caps[1].to_string();
                    let frag_query: Vec<(String, String)> =
                        frag_url.query_pairs().map(|(k, v)| (k.into_owned(), v.into_owned())).collect();
    ...
    if host.contains("zoom.us") || host.contains("zoomgov.com") {
        ...
        if !digits_only.is_match(&native_id) {
            return Err("Unsupported Zoom URL format. Expected https://zoom.us/j/<9-11 digit id>.".to_string());
        }
