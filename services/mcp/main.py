...
_KIOKU_ENV = os.getenv("KIOKU_ENV", "development")
_PUBLIC_DOCS = _KIOKU_ENV != "production"
app = FastAPI(
...
class RequestMeetingBot(BaseModel):
    meeting_url: Optional[str] = Field(
        None,
        description=(
            "Full meeting URL. If provided, Kioku will parse it and extract platform/native_meeting_id/passcode.\n"
...
async def parse_meeting_link(
    ...
    This is useful for agents: users can paste the full meeting URL, and Kioku will extract the
...
async def _get_prompt(name: str, arguments: Optional[Dict[str, str]] = None) -> mcp_types.GetPromptResult:
    ...
    if name == "vexa.meeting_prep":
        ...
        return mcp_types.GetPromptResult(
            description="Meeting prep flow using Kioku MCP tools.",
            messages=[
    ...
    if name == "vexa.during_meeting":
        ...
        meeting_id = (args.get("meeting_id") or "").strip()
        return mcp_types.GetPromptResult(
            description="During-meeting helper prompt using Kioku MCP tools.",
            messages=[
                mcp_types.PromptMessage(
                    role="user",
                    content=t(
    ...
    if name == "vexa.post_meeting":
        ...
        meeting_id = (args.get("meeting_id") or "").strip()
        return mcp_types.GetPromptResult(
            description="Post-meeting helper prompt using Kioku MCP tools.",
            messages=[
                mcp_types.PromptMessage(
                    role="user",
                    content=t(
