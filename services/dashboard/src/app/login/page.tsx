...
export default function LoginPage() {
  ...
  const isGithubAuthEnabled = (healthStatus?.checks as any)?.githubOAuth?.configured === true;
  const isMicrosoftAuthEnabled = healthStatus?.checks.azureAdOAuth?.configured === true;
  const isOAuthEnabled = isGoogleAuthEnabled || isGithubAuthEnabled || isMicrosoftAuthEnabled;
      ...
      {state === "email" ? (
        <div className="w-full max-w-sm">
          ...
          <div className="space-y-3">
            ...
            {/* GitHub Auth — only show when configured */}
            {isGithubAuthEnabled && (
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl border border-border bg-card hover:bg-accent hover:border-gray-300 transition-all text-left disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg className="h-5 w-5 flex-shrink-0" viewBox="0 0 98 96" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="flex-1">
                  <span className="text-sm font-medium text-foreground">Continue with GitHub</span>
                </div>
                <ArrowRight className="h-4 w-4 text-muted-foreground" />
              </button>
            )}
