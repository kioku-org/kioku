...
const DOCUMENT_CHUNK_SIZE: usize = 400;
const DOCUMENT_CHUNK_OVERLAP: usize = 80;
...
/// Chunk extracted document text (PDF, plain text, markdown, ...) into knowledge documents
/// with metadata. `chunk_type` is a display label (e.g. "pdf_document", "text_document").
pub fn chunk_document_text(
    ...
    let chunks = split_text(text, DOCUMENT_CHUNK_SIZE, DOCUMENT_CHUNK_OVERLAP);
...
mod tests {
    ...
    fn test_chunk_pdf_text() {
        ...
        let docs = chunk_document_text(&text, workspace_id, document_id, "test.pdf", "pdf_document");
