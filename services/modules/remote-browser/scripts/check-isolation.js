 ...
 * import of an undeclared module, is a hard failure. (Same gate as @vexa/join.)
 */
const fs = require("fs");
...
const pkgName = pkg.name || "this package";
...
if (violations.length) {
  console.error(`\n❌ ISOLATION BROKEN — ${violations.length} import(s) reach outside ${pkgName}:\n`);
  for (const v of violations) console.error("   " + v);
  console.error(`\nScanned ${fileCount} files. The remote-browser module must depend on nothing but its own src + declared npm deps.\n`);
  process.exit(1);
}
