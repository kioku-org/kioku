# Release validation report — `0.10.0-260427-1705`

_Generated 2026-04-27T14:26:35.023302Z from `tests3/.state/reports/`._
...
| `redis-client-robustness-layered` | compose, helm | lite `REDIS_CLIENT_HARDENED_TIMEOUTS`: ❌ fail<br>compose `REDIS_CLIENT_HARDENED_TIMEOUTS`: ❌ fail<br>helm `REDIS_CLIENT_HARDENED_TIMEOUTS`: ❌ fail<br>compose `MEETING_API_COLLECTOR_LIVENESS`: ❌ fail<br>compose `MEETING_API_READYZ_GATES_CONSUMER`: ❌ fail<br>compose `RUNTIME_API_REDIS_STARTUP_RETRY`: ⬜ missing<br>compose `MEETING_API_COLLECTOR_RECOVERS_AFTER_REDIS_KILL`: ⬜ missing<br>helm `REDIS_AOF_AND_BGSAVE_PAIRED`: ⬜ missing | **❌ fail** |
| `bot-lifecycle-orphan-pod-prevention` | compose | lite `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ❌ fail<br>compose `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ❌ fail<br>helm `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ❌ fail<br>compose `BOT_DELETE_DURABLE_RETRY`: ❌ fail<br>compose `BROWSER_SESSION_POD_TERMINATES_ON_NODE_EXIT`: ⬜ missing | **❌ fail** |
| `recording-durability-across-restart` | compose | compose `RECORDING_FINALIZE_SURVIVES_POD_RESTART`: ⬜ missing<br>lite `RECORDING_FINALIZE_OUTBOX_CONSUMER_IDEMPOTENT`: ❌ fail<br>compose `RECORDING_FINALIZE_OUTBOX_CONSUMER_IDEMPOTENT`: ❌ fail<br>lite `ZOOM_INCREMENTAL_UPLOAD_PRESENT`: ⬜ missing<br>compose `ZOOM_INCREMENTAL_UPLOAD_PRESENT`: ⬜ missing<br>lite `BOT_GRACEFUL_LEAVE_UPLOADS_FIRST`: ⬜ missing<br>compose `BOT_GRACEFUL_LEAVE_UPLOADS_FIRST`: ⬜ missing<br>lite `DELAYED_STOP_FINALIZER_REMOVED`: ❌ fail<br>compose `DELAYED_STOP_FINALIZER_REMOVED`: ❌ fail<br>compose `STALE_STOPPING_SWEEP_RECOVERS_MEETING`: ⬜ missing | **❌ fail** |
| `helm-prod-shape-smoke-matrix` | helm | helm `HELM_PROD_SHAPE_VALIDATES`: ⬜ missing | **⬜ missing** |
| `bot-pod-log-capture-compliance-coupled` | compose | lite `BOT_LOGS_STRUCTURED_JSON`: ❌ fail<br>compose `RUNTIME_API_CAPTURES_LOGS_PRE_REAP`: ⬜ missing<br>compose `BOT_LOGS_PURGED_WITH_MEETING`: ⬜ missing<br>compose `BOT_LOGS_BOUNDED_SIZE`: ⬜ missing | **⬜ missing** |
| `post-meeting-aggregation-state-machine` | compose | lite `TRANSIENT_5XX_RETRIED_AT_TRANSPORT_LAYER`: ⬜ missing<br>compose `TRANSIENT_5XX_RETRIED_AT_TRANSPORT_LAYER`: ⬜ missing<br>lite `AGGREGATION_FAILURE_CLASS_VIA_TYPED_HELPER`: ❌ fail<br>compose `AGGREGATION_FAILURE_CLASS_VIA_TYPED_HELPER`: ❌ fail<br>compose `AGGREGATION_FAILED_VS_USER_FAILED_DISTINCT`: ⬜ missing<br>compose `POST_MEETING_5XX_RETRIES_NOT_FAILS`: ⬜ missing | **❌ fail** |
...
| `slim-meeting-list-endpoint` | compose | lite `BOTS_LIST_ENDPOINT_SLIM`: ❌ fail<br>compose `BOTS_LIST_ENDPOINT_SLIM`: ❌ fail<br>compose `BOTS_LIST_PAYLOAD_SIZE_BOUNDED`: ⬜ missing | **❌ fail** |
...
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.0-260427-1705` | 11 | 9 | 2 |
| `helm` | `0.10.0-260427-1705` | 12 | 9 | 3 |
| `lite` | `0.10.0-260427-1705` | 10 | 8 | 2 |
...
| `containers` | ✅ pass | 108899 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 525 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1101 ms | 5 / 6 |
| `recording-survives-sigkill` | ✅ pass | 103 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 306 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 91488 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 622 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 4977 ms | 13 / 17 |
| `smoke-static` | ❌ fail | 430 ms | 51 / 64 |
| `webhooks` | ✅ pass | 96656 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `chart-all-services-db-pool-tuned` | ✅ pass | 321 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 562 ms | 4 / 4 |
| `chart-prod-secrets-secretref` | ✅ pass | 404 ms | 2 / 2 |
| `chart-rolling-update-zero-surge` | ✅ pass | 249 ms | 1 / 1 |
| `containers` | ❌ fail | 241856 ms | 5 / 7 |
| `dashboard-auth` | ✅ pass | 1662 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 3273 ms | 4 / 6 |
| `smoke-contract` | ❌ fail | 22972 ms | 25 / 27 |
| `smoke-env` | ✅ pass | 9930 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 34764 ms | 17 / 17 |
| `smoke-static` | ❌ fail | 735 ms | 53 / 64 |
| `webhooks` | ✅ pass | 103062 ms | 9 / 10 |
...
| `containers` | ❌ fail | 25434 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 479 ms | 4 / 4 |
| `package-tests` | ✅ pass | 95 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 286 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 39392 ms | 22 / 27 |
| `smoke-env` | ✅ pass | 850 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 5323 ms | 12 / 17 |
| `smoke-static` | ❌ fail | 521 ms | 51 / 64 |
| `webhooks` | ✅ pass | 96165 ms | 10 / 10 |
