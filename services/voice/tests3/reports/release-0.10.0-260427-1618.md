# Release validation report — `0.10.0-260427-1618`

_Generated 2026-04-27T13:35:26.274966Z from `tests3/.state/reports/`._

## Scope status

**Release**: `260427-k8s-stabilize` — **v0.10.5 — Production hardening release. ROBUSTNESS, not features.**
...
| `redis-client-robustness-layered` | compose, helm | lite `REDIS_CLIENT_HARDENED_TIMEOUTS`: ⬜ missing<br>compose `REDIS_CLIENT_HARDENED_TIMEOUTS`: ⬜ missing<br>helm `REDIS_CLIENT_HARDENED_TIMEOUTS`: ⬜ missing<br>compose `MEETING_API_COLLECTOR_LIVENESS`: ⬜ missing<br>compose `MEETING_API_READYZ_GATES_CONSUMER`: ⬜ missing<br>compose `RUNTIME_API_REDIS_STARTUP_RETRY`: ⬜ missing<br>compose `MEETING_API_COLLECTOR_RECOVERS_AFTER_REDIS_KILL`: ⬜ missing<br>helm `REDIS_AOF_AND_BGSAVE_PAIRED`: ⬜ missing | **⬜ missing** |
| `bot-lifecycle-orphan-pod-prevention` | compose | lite `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ⬜ missing<br>compose `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ⬜ missing<br>helm `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ⬜ missing<br>compose `BOT_DELETE_DURABLE_RETRY`: ⬜ missing<br>compose `BROWSER_SESSION_POD_TERMINATES_ON_NODE_EXIT`: ⬜ missing | **⬜ missing** |
| `recording-durability-across-restart` | compose | compose `RECORDING_FINALIZE_SURVIVES_POD_RESTART`: ⬜ missing<br>lite `RECORDING_FINALIZE_OUTBOX_CONSUMER_IDEMPOTENT`: ⬜ missing<br>compose `RECORDING_FINALIZE_OUTBOX_CONSUMER_IDEMPOTENT`: ⬜ missing<br>lite `ZOOM_INCREMENTAL_UPLOAD_PRESENT`: ⬜ missing<br>compose `ZOOM_INCREMENTAL_UPLOAD_PRESENT`: ⬜ missing<br>lite `BOT_GRACEFUL_LEAVE_UPLOADS_FIRST`: ⬜ missing<br>compose `BOT_GRACEFUL_LEAVE_UPLOADS_FIRST`: ⬜ missing<br>lite `DELAYED_STOP_FINALIZER_REMOVED`: ⬜ missing<br>compose `DELAYED_STOP_FINALIZER_REMOVED`: ⬜ missing<br>compose `STALE_STOPPING_SWEEP_RECOVERS_MEETING`: ⬜ missing | **⬜ missing** |
| `helm-prod-shape-smoke-matrix` | helm | helm `HELM_PROD_SHAPE_VALIDATES`: ⬜ missing | **⬜ missing** |
| `bot-pod-log-capture-compliance-coupled` | compose | lite `BOT_LOGS_STRUCTURED_JSON`: ⬜ missing<br>compose `RUNTIME_API_CAPTURES_LOGS_PRE_REAP`: ⬜ missing<br>compose `BOT_LOGS_PURGED_WITH_MEETING`: ⬜ missing<br>compose `BOT_LOGS_BOUNDED_SIZE`: ⬜ missing | **⬜ missing** |
| `post-meeting-aggregation-state-machine` | compose | lite `TRANSIENT_5XX_RETRIED_AT_TRANSPORT_LAYER`: ⬜ missing<br>compose `TRANSIENT_5XX_RETRIED_AT_TRANSPORT_LAYER`: ⬜ missing<br>lite `AGGREGATION_FAILURE_CLASS_VIA_TYPED_HELPER`: ⬜ missing<br>compose `AGGREGATION_FAILURE_CLASS_VIA_TYPED_HELPER`: ⬜ missing<br>compose `AGGREGATION_FAILED_VS_USER_FAILED_DISTINCT`: ⬜ missing<br>compose `POST_MEETING_5XX_RETRIES_NOT_FAILS`: ⬜ missing | **⬜ missing** |
...
| `slim-meeting-list-endpoint` | compose | lite `BOTS_LIST_ENDPOINT_SLIM`: ⬜ missing<br>compose `BOTS_LIST_ENDPOINT_SLIM`: ⬜ missing<br>compose `BOTS_LIST_PAYLOAD_SIZE_BOUNDED`: ⬜ missing | **⬜ missing** |
...
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.0-260427-1618` | 11 | 9 | 2 |
| `helm` | `0.10.0-260427-1618` | 6 | 5 | 1 |
| `lite` | `0.10.0-260427-1618` | 10 | 8 | 2 |
...
| `bot-lifecycle` | **44%** | 90% | ❌ below gate |
| `browser-session` | **0%** | 0% | ✅ pass |
| `container-lifecycle` | **0%** | 0% | ✅ pass |
| `dashboard` | **32%** | 90% | ❌ below gate |
| `infrastructure` | **50%** | 100% | ❌ below gate |
...
| `containers` | ✅ pass | 108993 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 541 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1121 ms | 5 / 6 |
| `recording-survives-sigkill` | ✅ pass | 111 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 269 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 91559 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 622 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5283 ms | 13 / 17 |
| `smoke-static` | ❌ fail | 467 ms | 49 / 54 |
| `webhooks` | ✅ pass | 96729 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `chart-all-services-db-pool-tuned` | ✅ pass | 324 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 586 ms | 4 / 4 |
| `chart-prod-secrets-secretref` | ✅ pass | 393 ms | 2 / 2 |
| `chart-rolling-update-zero-surge` | ✅ pass | 240 ms | 1 / 1 |
| `smoke-env` | ✅ pass | 3576 ms | 7 / 7 |
| `smoke-static` | ❌ fail | 728 ms | 50 / 54 |
...
| `containers` | ❌ fail | 25425 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 462 ms | 4 / 4 |
| `package-tests` | ✅ pass | 84 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 293 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 39619 ms | 22 / 27 |
| `smoke-env` | ✅ pass | 788 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 5405 ms | 12 / 17 |
| `smoke-static` | ❌ fail | 493 ms | 49 / 54 |
| `webhooks` | ✅ pass | 96424 ms | 10 / 10 |
