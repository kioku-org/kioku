# Release validation report — `0.10.0-260427-1734`

_Generated 2026-04-27T15:00:04.762320Z from `tests3/.state/reports/`._

## Scope status

**Release**: `260427` — **v0.10.5 — Production hardening release. ROBUSTNESS, not features.**

| Issue | Required modes | Status per proof | Verdict |
|-------|----------------|-------------------|---------|
| `chart-deploy-shape-restored` | helm | helm `HELM_TOLERATIONS_PROPAGATED`: ⬜ missing<br>helm `HELM_UPGRADE_TWICE_NO_JOB_IMMUTABLE`: ⬜ missing<br>helm `HELM_FRESH_INSTALL_BUCKETS_PRESENT`: ⬜ missing<br>helm `HELM_CAPACITY_RESERVE_ZERO`: ⬜ missing<br>helm `HELM_NETWORK_POLICY_TX_GATEWAY`: ⬜ missing<br>helm `HELM_NO_HARDCODED_RELEASE_NAME`: ⬜ missing | **⬜ missing** |
| `bot-pod-scheduling-and-storage-env` | compose, helm | lite `RUNTIME_PROFILE_SCHEMA_ACCEPTS_K8S_KEYS`: ⬜ missing<br>compose `RUNTIME_PROFILE_SCHEMA_ACCEPTS_K8S_KEYS`: ⬜ missing<br>helm `RUNTIME_PROFILE_SCHEMA_ACCEPTS_K8S_KEYS`: ⬜ missing<br>helm `BOT_POD_SCHEDULING_FROM_PROFILE`: ⬜ missing<br>helm `BOT_POD_RECORDING_ENV_PROPAGATED`: ⬜ missing | **⬜ missing** |
| `redis-client-robustness-layered` | compose, helm | lite `REDIS_CLIENT_HARDENED_TIMEOUTS`: ✅ pass<br>compose `REDIS_CLIENT_HARDENED_TIMEOUTS`: ✅ pass<br>helm `REDIS_CLIENT_HARDENED_TIMEOUTS`: ✅ pass<br>compose `MEETING_API_COLLECTOR_LIVENESS`: ✅ pass<br>compose `MEETING_API_READYZ_GATES_CONSUMER`: ✅ pass<br>compose `RUNTIME_API_REDIS_STARTUP_RETRY`: ⬜ missing<br>compose `MEETING_API_COLLECTOR_RECOVERS_AFTER_REDIS_KILL`: ⬜ missing<br>helm `REDIS_AOF_AND_BGSAVE_PAIRED`: ⬜ missing | **⬜ missing** |
| `bot-lifecycle-orphan-pod-prevention` | compose | lite `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ✅ pass<br>compose `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ✅ pass<br>helm `K8S_BACKEND_CONTAINER_ID_IS_NAME`: ✅ pass<br>compose `BOT_DELETE_DURABLE_RETRY`: ✅ pass<br>compose `BROWSER_SESSION_POD_TERMINATES_ON_NODE_EXIT`: ⬜ missing | **⬜ missing** |
| `recording-durability-across-restart` | compose | compose `RECORDING_FINALIZE_SURVIVES_POD_RESTART`: ⬜ missing<br>lite `RECORDING_FINALIZE_OUTBOX_CONSUMER_IDEMPOTENT`: ✅ pass<br>compose `RECORDING_FINALIZE_OUTBOX_CONSUMER_IDEMPOTENT`: ✅ pass<br>lite `ZOOM_INCREMENTAL_UPLOAD_PRESENT`: ⬜ missing<br>compose `ZOOM_INCREMENTAL_UPLOAD_PRESENT`: ⬜ missing<br>lite `BOT_GRACEFUL_LEAVE_UPLOADS_FIRST`: ⬜ missing<br>compose `BOT_GRACEFUL_LEAVE_UPLOADS_FIRST`: ⬜ missing<br>lite `DELAYED_STOP_FINALIZER_REMOVED`: ✅ pass<br>compose `DELAYED_STOP_FINALIZER_REMOVED`: ✅ pass<br>compose `STALE_STOPPING_SWEEP_RECOVERS_MEETING`: ⬜ missing | **⬜ missing** |
| `helm-prod-shape-smoke-matrix` | helm | helm `HELM_PROD_SHAPE_VALIDATES`: ⬜ missing | **⬜ missing** |
| `bot-pod-log-capture-compliance-coupled` | compose | lite `BOT_LOGS_STRUCTURED_JSON`: ✅ pass<br>compose `RUNTIME_API_CAPTURES_LOGS_PRE_REAP`: ⬜ missing<br>compose `BOT_LOGS_PURGED_WITH_MEETING`: ⬜ missing<br>compose `BOT_LOGS_BOUNDED_SIZE`: ⬜ missing | **⬜ missing** |
| `post-meeting-aggregation-state-machine` | compose | lite `TRANSIENT_5XX_RETRIED_AT_TRANSPORT_LAYER`: ⬜ missing<br>compose `TRANSIENT_5XX_RETRIED_AT_TRANSPORT_LAYER`: ⬜ missing<br>lite `AGGREGATION_FAILURE_CLASS_VIA_TYPED_HELPER`: ✅ pass<br>compose `AGGREGATION_FAILURE_CLASS_VIA_TYPED_HELPER`: ✅ pass<br>compose `AGGREGATION_FAILED_VS_USER_FAILED_DISTINCT`: ⬜ missing<br>compose `POST_MEETING_5XX_RETRIES_NOT_FAILS`: ⬜ missing | **⬜ missing** |
| `chart-availability-defaults-stateless` | helm | helm `CHART_STATELESS_SERVICES_REPLICACOUNT_AT_LEAST_2`: ⬜ missing<br>helm `CHART_STATELESS_SERVICES_HAVE_PDB`: ⬜ missing<br>helm `CHART_STATELESS_SERVICES_HAVE_PROBES`: ⬜ missing<br>helm `CHART_STATELESS_SERVICES_GRACE_PERIOD`: ⬜ missing | **⬜ missing** |
| `bot-exit-classification-refinement` | compose | lite `BOT_ABNORMAL_EXIT_CATALOG_COMPLETE`: ⬜ missing<br>compose `BOT_ABNORMAL_EXIT_CATALOG_COMPLETE`: ⬜ missing<br>compose `BOT_EXIT_CLASSIFICATION_REQUIRES_POSITIVE_SUCCESS`: ⬜ missing<br>compose `BOT_CLEAN_EXIT_NOT_MISCLASSIFIED_FAILED`: ⬜ missing | **⬜ missing** |
| `browser-session-idle-eviction` | compose | compose `BROWSER_SESSION_IDLE_EVICTION_FIRES`: ⬜ missing<br>compose `BROWSER_SESSION_K8S_VS_REDIS_RECONCILED`: ⬜ missing | **⬜ missing** |
| `slim-meeting-list-endpoint` | compose | lite `BOTS_LIST_ENDPOINT_SLIM`: ✅ pass<br>compose `BOTS_LIST_ENDPOINT_SLIM`: ✅ pass<br>compose `BOTS_LIST_PAYLOAD_SIZE_BOUNDED`: ⬜ missing | **⬜ missing** |
| `meetings-data-row-size-monitoring` | helm | compose `MEETINGS_DATA_ROW_SIZE_METRIC_EXPORTED`: ⬜ missing<br>helm `MEETINGS_DATA_ROW_SIZE_METRIC_EXPORTED`: ⬜ missing | **⬜ missing** |
| `bot-server-contract-parity-checks` | compose, helm, lite | lite `BOT_SERVER_CONFIG_CONTRACT_PARITY`: ⬜ missing<br>compose `BOT_SERVER_CONFIG_CONTRACT_PARITY`: ⬜ missing<br>helm `BOT_SERVER_CONFIG_CONTRACT_PARITY`: ⬜ missing<br>lite `MEETING_STATUS_ENUM_CROSS_SERVICE_PARITY`: ⬜ missing<br>compose `MEETING_STATUS_ENUM_CROSS_SERVICE_PARITY`: ⬜ missing<br>helm `MEETING_STATUS_ENUM_CROSS_SERVICE_PARITY`: ⬜ missing | **⬜ missing** |
| `make-all-non-interactive-shell` | compose | compose `MAKE_ALL_NON_INTERACTIVE_SHELL_EXITS_CLEAN`: ⬜ missing | **⬜ missing** |

## Deployment coverage

| Mode | Image tag | Tests run | Passed | Failed |
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.0-260427-1734` | 11 | 10 | 1 |
| `helm` | `0.10.0-260427-1734` | 12 | 9 | 3 |
| `lite` | `0.10.0-260427-1734` | 10 | 9 | 1 |
...
|------|:------:|---------:|---------------------:|
| `bot-records-incrementally` | ✅ pass | 97 ms | 1 / 1 |
| `containers` | ✅ pass | 108892 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 524 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1100 ms | 5 / 6 |
| `recording-survives-sigkill` | ✅ pass | 98 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 309 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 91654 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 623 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5213 ms | 13 / 17 |
| `smoke-static` | ✅ pass | 391 ms | 63 / 64 |
| `webhooks` | ✅ pass | 96635 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `chart-all-services-db-pool-tuned` | ✅ pass | 278 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 568 ms | 4 / 4 |
| `chart-prod-secrets-secretref` | ✅ pass | 334 ms | 2 / 2 |
| `chart-rolling-update-zero-surge` | ✅ pass | 249 ms | 1 / 1 |
| `containers` | ✅ pass | 114420 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 1529 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 3379 ms | 5 / 6 |
| `smoke-contract` | ❌ fail | 116198 ms | 23 / 27 |
| `smoke-env` | ✅ pass | 9215 ms | 7 / 7 |
| `smoke-health` | ❌ fail | 35045 ms | 16 / 17 |
| `smoke-static` | ✅ pass | 718 ms | 63 / 64 |
| `webhooks` | ❌ fail | 104985 ms | 9 / 10 |
...
| `containers` | ❌ fail | 25401 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 471 ms | 4 / 4 |
| `package-tests` | ✅ pass | 93 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 304 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 39224 ms | 22 / 27 |
| `smoke-env` | ✅ pass | 814 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 5525 ms | 12 / 17 |
| `smoke-static` | ✅ pass | 478 ms | 63 / 64 |
| `webhooks` | ✅ pass | 96113 ms | 10 / 10 |
