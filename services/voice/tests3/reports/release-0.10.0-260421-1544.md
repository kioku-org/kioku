# Release validation report — `0.10.0-260421-1544`

_Generated 2026-04-21T13:01:37.685111Z from `tests3/.state/reports/`._
...
| `bot-recording-incremental-chunk-upload` | compose, helm | compose `RECORDING_UPLOAD_SUPPORTS_CHUNK_SEQ`: ✅ pass<br>lite `BOT_RECORDS_INCREMENTALLY`: ⬜ missing<br>compose `BOT_RECORDS_INCREMENTALLY`: ⬜ missing<br>compose `RECORDING_SURVIVES_MID_MEETING_KILL`: ⬜ missing<br>helm `RUNTIME_API_STOP_GRACE_MATCHES_POD_SPEC`: ✅ pass | **⬜ missing** |
| `transcript-rendering-dedup-prefers-confirmed-on-containment` | lite | lite `TRANSCRIPT_RENDERING_DEDUP_TESTS_PASS`: ⬜ missing<br>lite `PACKAGES_CI_WORKFLOW_EXISTS`: ✅ pass | **⬜ missing** |
| `engine-pool-reset-on-return-guarded` | compose, helm, lite | lite `ENGINE_POOL_RESET_ON_RETURN_ROLLBACK`: ✅ pass<br>compose `ENGINE_POOL_RESET_ON_RETURN_ROLLBACK`: ✅ pass<br>helm `HELM_ALL_SERVICES_DB_POOL_TUNED`: ⬜ missing<br>compose `DB_POOL_NO_EXHAUSTION`: ✅ pass | **⬜ missing** |
| `chart-rolling-update-zero-pool-overlap` | helm | helm `HELM_DEPLOYMENT_STRATEGY_HELPER_DEFINED`: ✅ pass<br>helm `HELM_ROLLING_UPDATE_ZERO_SURGE`: ⬜ missing<br>helm `HELM_API_GATEWAY_REPLICA_COUNT_HA`: ✅ pass | **⬜ missing** |
| `pgbouncer-as-optional-oss-chart-component` | helm | helm `HELM_PGBOUNCER_OPTIONAL_AND_WIRED`: ⬜ missing | **⬜ missing** |
| `runtime-api-exit-callback-delivery-is-durable` | compose | lite `RUNTIME_API_IDLE_LOOP_SWEEPS_PENDING_CALLBACKS`: ✅ pass<br>compose `RUNTIME_API_IDLE_LOOP_SWEEPS_PENDING_CALLBACKS`: ✅ pass<br>compose `RUNTIME_API_EXIT_CALLBACK_DURABLE`: ⬜ missing | **⬜ missing** |

## Deployment coverage

| Mode | Image tag | Tests run | Passed | Failed |
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.0-260421-1544` | 11 | 9 | 2 |
| `helm` | `0.10.0-260421-1544` | 12 | 10 | 2 |
| `lite` | `0.10.0-260421-1544` | 10 | 6 | 4 |
...
| `security-hygiene` | **58%** | 95% | ❌ below gate |
...
|------|:------:|---------:|---------------------:|
| `bot-records-incrementally` | ✅ pass | 93 ms | 1 / 1 |
| `containers` | ✅ pass | 109184 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 506 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1182 ms | 5 / 6 |
| `recording-survives-sigkill` | ✅ pass | 92 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ❌ fail | 239 ms | 2 / 3 |
| `smoke-contract` | ✅ pass | 91673 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 633 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5265 ms | 13 / 17 |
| `smoke-static` | ✅ pass | 25 ms | 40 / 41 |
| `webhooks` | ✅ pass | 96813 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `chart-all-services-db-pool-tuned` | ✅ pass | 314 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 476 ms | 3 / 3 |
| `chart-prod-secrets-secretref` | ✅ pass | 395 ms | 2 / 2 |
| `chart-rolling-update-zero-surge` | ✅ pass | 246 ms | 1 / 1 |
| `containers` | ❌ fail | 11622 ms | 1 / 2 |
| `dashboard-auth` | ✅ pass | 1491 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1706 ms | 3 / 6 |
| `smoke-contract` | ❌ fail | 82512 ms | 23 / 27 |
| `smoke-env` | ✅ pass | 9611 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 34613 ms | 17 / 17 |
| `smoke-static` | ✅ pass | 579 ms | 40 / 41 |
| `webhooks` | ✅ pass | 92617 ms | 9 / 10 |
...
| `containers` | ❌ fail | 25437 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 484 ms | 4 / 4 |
| `package-tests` | ❌ fail | 33 ms | 0 / 0 |
| `runtime-api-exit-callback-durable` | ❌ fail | 209 ms | 2 / 3 |
| `smoke-contract` | ❌ fail | 64248 ms | 21 / 27 |
| `smoke-env` | ✅ pass | 743 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 5448 ms | 12 / 17 |
| `smoke-static` | ✅ pass | 73 ms | 40 / 41 |
| `webhooks` | ✅ pass | 96342 ms | 10 / 10 |
