# Release validation report — `0.10.0-260421-1502`

_Generated 2026-04-21T12:26:41.392786Z from `tests3/.state/reports/`._

## Scope status

**Release**: `260421-prod-stabilize` — Post-incident 0.10.x stabilization, audit-reduced scope. Seven issues,

| Issue | Required modes | Status per proof | Verdict |
|-------|----------------|-------------------|---------|
| `chart-every-prod-secret-via-secretkeyref` | helm | helm `HELM_PROD_SECRETS_SECRETREF_ONLY`: ⬜ missing<br>helm `HELM_PROD_SECRETS_REQUIRED_AT_RENDER`: ⬜ missing | **⬜ missing** |
| `bot-recording-incremental-chunk-upload` | compose, helm | compose `RECORDING_UPLOAD_SUPPORTS_CHUNK_SEQ`: ⬜ missing<br>lite `BOT_RECORDS_INCREMENTALLY`: ⬜ missing<br>compose `BOT_RECORDS_INCREMENTALLY`: ⬜ missing<br>compose `RECORDING_SURVIVES_MID_MEETING_KILL`: ⬜ missing<br>helm `RUNTIME_API_STOP_GRACE_MATCHES_POD_SPEC`: ⬜ missing | **⬜ missing** |
| `transcript-rendering-dedup-prefers-confirmed-on-containment` | lite | lite `TRANSCRIPT_RENDERING_DEDUP_TESTS_PASS`: ⬜ missing<br>lite `PACKAGES_CI_WORKFLOW_EXISTS`: ⬜ missing | **⬜ missing** |
| `engine-pool-reset-on-return-guarded` | compose, helm, lite | lite `ENGINE_POOL_RESET_ON_RETURN_ROLLBACK`: ⬜ missing<br>compose `ENGINE_POOL_RESET_ON_RETURN_ROLLBACK`: ⬜ missing<br>helm `HELM_ALL_SERVICES_DB_POOL_TUNED`: ⬜ missing<br>compose `DB_POOL_NO_EXHAUSTION`: ✅ pass | **⬜ missing** |
| `chart-rolling-update-zero-pool-overlap` | helm | helm `HELM_DEPLOYMENT_STRATEGY_HELPER_DEFINED`: ⬜ missing<br>helm `HELM_ROLLING_UPDATE_ZERO_SURGE`: ⬜ missing<br>helm `HELM_API_GATEWAY_REPLICA_COUNT_HA`: ⬜ missing | **⬜ missing** |
| `pgbouncer-as-optional-oss-chart-component` | helm | helm `HELM_PGBOUNCER_OPTIONAL_AND_WIRED`: ⬜ missing | **⬜ missing** |
| `runtime-api-exit-callback-delivery-is-durable` | compose | lite `RUNTIME_API_IDLE_LOOP_SWEEPS_PENDING_CALLBACKS`: ⬜ missing<br>compose `RUNTIME_API_IDLE_LOOP_SWEEPS_PENDING_CALLBACKS`: ⬜ missing<br>compose `RUNTIME_API_EXIT_CALLBACK_DURABLE`: ⬜ missing | **⬜ missing** |

## Deployment coverage

| Mode | Image tag | Tests run | Passed | Failed |
|------|-----------|-----------|--------|--------|
| `compose` | `0.10.0-260419-1910` | 9 | 8 | 1 |
| `helm` | `0.10.0-260421-1502` | 8 | 7 | 1 |
| `lite` | `0.10.0-260419-1910` | 7 | 3 | 4 |
...
| `security-hygiene` | **42%** | 95% | ❌ below gate |
...
| bots-status-not-422 | GET /bots/status never returns 422 (schema stable under concurrent writes) | 5 | ❌ fail | lite: smoke-contract/BOTS_STATUS_NOT_422: HTTP 401 (expected 200); compose: smoke-contract/BOTS_STATUS_NOT_422: GET /bots/status returns 200 — no route collision with /bots/{meeting_id}; helm: smok… |
...
| admin-api-up | admin-api responds with a valid list | 10 | ❌ fail | lite: smoke-health/ADMIN_API_UP: HTTP 500 (expected 200); compose: smoke-health/ADMIN_API_UP: admin-api responds with valid token — user management and login work; helm: smoke-health/ADMIN_API_UP: … |
...
| invalid-rejected | Invalid meeting URL returns 400 (not 500) | 10 | ❌ fail | lite: smoke-contract/INVALID_URL_REJECTED: HTTP 401 (expected one of [400, 422]); compose: smoke-contract/INVALID_URL_REJECTED: garbage URLs rejected with 400/422 — input validation works; helm: sm… |
...
| chart-prod-secrets-via-secretkeyref | every prod secret (DB_PASSWORD, TRANSCRIPTION_SERVICE_TOKEN) sourced via secretKeyRef in rendered chart | 15 | ⬜ missing | helm: check HELM_PROD_SECRETS_SECRETREF_ONLY not found in any smoke-* report |
| chart-prod-secrets-required-at-render | helm template exits non-zero when prod secrets are absent — fail loud at render, not silently at pod boot | 10 | ⬜ missing | helm: check HELM_PROD_SECRETS_REQUIRED_AT_RENDER not found in any smoke-* report |
| engine-pool-reset-on-return-rollback-explicit | meeting-api database.py engine sets pool_reset_on_return='rollback' — regression guard for idle-in-transaction leak defense | 10 | ⬜ missing | lite: check ENGINE_POOL_RESET_ON_RETURN_ROLLBACK not found in any smoke-* report; compose: check ENGINE_POOL_RESET_ON_RETURN_ROLLBACK not found in any smoke-* report |
...
|------|:------:|---------:|---------------------:|
| `bot-records-incrementally` | ✅ pass | 131 ms | 1 / 1 |
| `containers` | ✅ pass | 109215 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 514 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1120 ms | 5 / 6 |
| `smoke-contract` | ✅ pass | 92063 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 646 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5407 ms | 13 / 17 |
| `smoke-static` | ✅ pass | 25 ms | 33 / 34 |
| `webhooks` | ✅ pass | 96933 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `containers` | ✅ pass | 116184 ms | 7 / 7 |
| `dashboard-auth` | ✅ pass | 1533 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 3218 ms | 4 / 6 |
| `smoke-contract` | ❌ fail | 81430 ms | 24 / 27 |
| `smoke-env` | ✅ pass | 9651 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 35150 ms | 17 / 17 |
| `smoke-static` | ✅ pass | 580 ms | 33 / 34 |
| `webhooks` | ✅ pass | 92679 ms | 9 / 10 |
...
| `smoke-contract` | ❌ fail | 8336 ms | 6 / 27 |
| `smoke-env` | ✅ pass | 724 ms | 7 / 7 |
| `smoke-health` | ❌ fail | 5125 ms | 7 / 17 |
| `smoke-static` | ✅ pass | 75 ms | 33 / 34 |
