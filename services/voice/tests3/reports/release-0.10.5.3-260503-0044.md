# Release validation report — `0.10.5.3-260503-0044`

_Generated 2026-05-02T22:05:44.952126Z from `tests3/.state/reports/`._
...
| `compose` | `0.10.5.3-260503-0044` | 28 | 27 | 1 |
| `helm` | `0.10.5.3-260503-0044` | 15 | 12 | 3 |
| `lite` | `0.10.5.3-260503-0044` | 12 | 9 | 3 |
...
| `dashboard` | **86%** | 90% | ❌ below gate |
| `infrastructure` | **68%** | 100% | ❌ below gate |
...
| bots-status-not-422 | GET /bots/status never returns 422 (schema stable under concurrent writes) | 5 | ❌ fail | lite: smoke-contract/BOTS_STATUS_NOT_422: HTTP 0 (expected 200); compose: smoke-contract/BOTS_STATUS_NOT_422: GET /bots/status returns 200 — no route collision with /bots/{meeting_id}; helm: smoke-… |
...
| dashboard-up | Dashboard root page responds | 5 | ❌ fail | lite: smoke-health/DASHBOARD_UP: HTTP 0 (expected 200); compose: smoke-health/DASHBOARD_UP: dashboard serves pages — user can access the UI; helm: smoke-health/DASHBOARD_UP: dashboard serves pages … |
| dashboard-ws-url | NEXT_PUBLIC_WS_URL is set — live updates can connect | 5 | ❌ fail | lite: smoke-health/DASHBOARD_WS_URL: HTTP 0 from http://localhost:3000/api/config; compose: smoke-health/DASHBOARD_WS_URL: ws://localhost:3001/ws; helm: smoke-health/DASHBOARD_WS_URL: ws://172.232.… |
...
### `infrastructure` (68% / gate 100%)

| # | Label | Weight | Status | Evidence |
|---|-------|-------:|:------:|----------|
| gateway-up | API gateway responds to /admin/users via valid admin token | 10 | ❌ fail | lite: smoke-health/GATEWAY_UP: HTTP 0 (expected 200); compose: smoke-health/GATEWAY_UP: API gateway accepts connections — all client requests can reach backend; helm: smoke-health/GATEWAY_UP: API g… |
| admin-api-up | admin-api responds with a valid list | 10 | ❌ fail | lite: smoke-health/ADMIN_API_UP: HTTP 0 (expected 200); compose: smoke-health/ADMIN_API_UP: admin-api responds with valid token — user management and login work; helm: smoke-health/ADMIN_API_UP: ad… |
| dashboard-up | dashboard root page responds | 10 | ❌ fail | lite: smoke-health/DASHBOARD_UP: HTTP 0 (expected 200); compose: smoke-health/DASHBOARD_UP: dashboard serves pages — user can access the UI; helm: smoke-health/DASHBOARD_UP: dashboard serves pages … |
| runtime-api-up | runtime-api (bot orchestrator) is reachable / has ready replicas | 15 | ❌ fail | lite: smoke-health/RUNTIME_API_UP: HTTP 0 from localhost:8090/health; compose: smoke-health/RUNTIME_API_UP: runtime-api responds — bot container lifecycle management works; helm: smoke-health/RUNTI… |
...
| redis-up | Redis responds to PING | 10 | ❌ fail | lite: smoke-health/REDIS_UP: redis-cli ping: ; compose: smoke-health/REDIS_UP: Redis responds to PING — WebSocket pub/sub, session state, and caching work; helm: smoke-health/REDIS_UP: Redis respon… |
...
| gmeet-parsed | Google Meet URL (meet.google.com/xxx-xxxx-xxx) parses correctly | 15 | ❌ fail | lite: smoke-contract/GMEET_URL_PARSED: HTTP 0 (expected one of [200, 201, 202, 403, 409, 500]); compose: smoke-contract/GMEET_URL_PARSED: Google Meet URL accepted by POST /bots — parser handles GMe… |
| invalid-rejected | Invalid meeting URL returns 400 (not 500) | 10 | ❌ fail | lite: smoke-contract/INVALID_URL_REJECTED: HTTP 0 (expected one of [400, 422]); compose: smoke-contract/INVALID_URL_REJECTED: garbage URLs rejected with 400/422 — input validation works; helm: smok… |
| teams-standard | Teams standard link (teams.microsoft.com/l/meetup-join/...) parses | 15 | ❌ fail | lite: smoke-contract/TEAMS_URL_STANDARD: HTTP 0 (expected one of [200, 201, 202, 403, 409, 500]); compose: smoke-contract/TEAMS_URL_STANDARD: Teams standard join URL accepted by POST /bots; helm: s… |
| teams-shortlink | Teams shortlink (teams.live.com, teams.microsoft.com/meet) parses | 10 | ❌ fail | lite: smoke-contract/TEAMS_URL_SHORTLINK: HTTP 0 (expected one of [200, 201, 202, 403, 409, 500]); compose: smoke-contract/TEAMS_URL_SHORTLINK: Teams /meet/ shortlink URL parsed and accepted by POS… |
| teams-channel | Teams channel meeting URL parses | 10 | ❌ fail | lite: smoke-contract/TEAMS_URL_CHANNEL: HTTP 0 (expected one of [200, 201, 202, 403, 409, 422, 500]); compose: smoke-contract/TEAMS_URL_CHANNEL: Teams channel meeting URL accepted or known gap; hel… |
| teams-enterprise | Teams enterprise-tenant URL parses (custom domain) | 15 | ❌ fail | lite: smoke-contract/TEAMS_URL_ENTERPRISE: HTTP 0 (expected one of [200, 201, 202, 403, 409, 500]); compose: smoke-contract/TEAMS_URL_ENTERPRISE: Teams enterprise domain URL parsed and accepted by … |
| teams-personal | Teams personal-account URL parses | 15 | ❌ fail | lite: smoke-contract/TEAMS_URL_PERSONAL: HTTP 0 (expected one of [200, 201, 202, 403, 409, 500]); compose: smoke-contract/TEAMS_URL_PERSONAL: Teams personal (teams.live.com) URL parsed and accepted… |
...
| `containers` | ✅ pass | 109090 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 513 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 1113 ms | 5 / 6 |
| `recording-survives-sigkill` | ✅ pass | 88 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 282 ms | 4 / 4 |
| `smoke-contract` | ✅ pass | 91629 ms | 27 / 27 |
| `smoke-env` | ❌ fail | 583 ms | 6 / 7 |
| `smoke-health` | ✅ pass | 5035 ms | 13 / 17 |
| `smoke-static` | ✅ pass | 592 ms | 91 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 251 ms | 3 / 3 |
...
| `v0.10.6-static-greps` | ✅ pass | 1028 ms | 15 / 15 |
...
| `webhooks` | ✅ pass | 96890 ms | 10 / 10 |

### `helm`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `chart-all-services-db-pool-tuned` | ✅ pass | 320 ms | 1 / 1 |
| `chart-pgbouncer-optional` | ✅ pass | 566 ms | 4 / 4 |
| `chart-prod-secrets-secretref` | ✅ pass | 397 ms | 2 / 2 |
| `chart-rolling-update-zero-downtime` | ✅ pass | 248 ms | 1 / 1 |
| `chart-rolling-update-zero-surge` | ❌ fail | 253 ms | 0 / 1 |
| `containers` | ✅ pass | 125150 ms | 6 / 7 |
| `dashboard-auth` | ✅ pass | 1749 ms | 4 / 4 |
| `dashboard-proxy` | ✅ pass | 3371 ms | 5 / 6 |
| `smoke-contract` | ❌ fail | 125298 ms | 23 / 27 |
| `smoke-env` | ✅ pass | 9216 ms | 7 / 7 |
| `smoke-health` | ✅ pass | 33832 ms | 17 / 17 |
| `smoke-static` | ✅ pass | 2977 ms | 91 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 328 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 1357 ms | 15 / 15 |
| `webhooks` | ❌ fail | 105017 ms | 9 / 10 |

### `lite`

| Test | Status | Duration | Steps (pass / total) |
|------|:------:|---------:|---------------------:|
| `bot-records-incrementally` | ✅ pass | 114 ms | 1 / 1 |
| `containers` | ❌ fail | 25427 ms | 2 / 2 |
| `dashboard-auth` | ✅ pass | 495 ms | 4 / 4 |
| `package-tests` | ✅ pass | 109 ms | 1 / 1 |
| `runtime-api-exit-callback-durable` | ✅ pass | 698 ms | 4 / 4 |
| `smoke-contract` | ❌ fail | 2385 ms | 3 / 27 |
| `smoke-env` | ✅ pass | 399 ms | 7 / 7 |
| `smoke-health` | ❌ fail | 1212 ms | 2 / 17 |
| `smoke-static` | ✅ pass | 556 ms | 91 / 92 |
| `v0.10.5.3-hallucination-corpus` | ✅ pass | 251 ms | 3 / 3 |
| `v0.10.6-static-greps` | ✅ pass | 954 ms | 15 / 15 |
| `webhooks` | ✅ pass | 96288 ms | 10 / 10 |
